...
class Extractor:
    ...
    def extractFromLog(self):
        ...
        items = self.mongo.db.question_log_old.find({}, {"log": 1})
        ...
        self.mongo.db.editors_old.insert_many(self.user_list)
    ...
    def extractFromAnswer(self):
        ...
        items = self.mongo.db.question_answers_old.find({})
        for item in items:
            print '第%d个'% count
            count = count + 1
        ...
        #self.mongo.insertToUsersByFilter(self.user_list)
        self.mongo.db.answerers_old.insert_many(self.user_list)
    ...
    def extractQuestionFollowers(self):
        ...
        items = self.mongo.db.question_followers_old.find({}, {"follower_num": 1, "followers": 1 })
        for item in items:
            print '第%d个' % count
            count = count + 1
        ...
        #self.mongo.insertToUsersByFilter(self.user_list)
        self.mongo.db.followers_old.insert_many(self.user_list)
    ...
    def extractVoters(self):
        ...
        items = self.mongo.db.answer_voters_old.find({}, {'voter_num': 1, 'voters': 1})
        for item in items:
            print '第%d个回答' % count
            count += 1
        ...
        #self.mongo.insertToUsersByFilter(self.user_list)
        self.mongo.db.voters_old.insert_many(self.user_list)
    ...
    def extractCommenters(self):
        ...
        items = self.mongo.db.answer_comments_old.find({}, {'comment_num': 1, 'comments': 1})
        for item in items:
            print '第%d个评论...' % count
            count += 1
        ...
        #self.mongo.insertToUsersByFilter(self.user_list)
        self.mongo.db.commenters_old.insert_many(self.user_list)
